...
public class TransportPacket {
	...
	private boolean verifyMAC(DataInputStream iStream,byte[] data) throws Exception{
		mac=new byte[session.HASHS2C.getBlockSize()];
		iStream.readFully(mac);
		session.HASHS2C.update(session.seqIn);
		session.HASHS2C.update(data,0,data.length);
		byte[] checkData=new byte[mac.length];
		session.HASHS2C.doFinal(checkData, 0);
		//session.seqIn++;
		return java.util.Arrays.equals(mac, checkData);		
	}
	public void readPacket(InputStream iStream) throws SocketException{
		try{
			if (session.cipherS2C==null) //no encrypted
				...
				session.cipherS2C.update(firstBlock, 0, session.cipherBlocksize, firstBlock, 0);
				...
				session.cipherS2C.update(data, session.cipherBlocksize, remaining, data, session.cipherBlocksize);
	...
	private void readPacket_noEncrypt(InputStream iStream) throws SocketException{
		try{
	...
	private byte[] calMac(byte[] data){
		try{
			byte[] hash=new byte[session.HASHC2S.getBlockSize()];
			session.HASHC2S.update(session.seqOut);
			session.HASHC2S.update(data,0,data.length);
			session.HASHC2S.doFinal(hash,0);
			//session.seqOut++;
			return hash;
		}
		catch (Exception e){
	...
	public void send(OutputStream stream) throws SocketException{
		if (session.cipherC2S==null)
			...
			try{
				...
				if (session.HASHC2S!=null)
					mac=calMac(data);
				session.cipherC2S.update(data, 0, data.length, data, 0);
				if (session.HASHC2S!=null){
	...
	private void send_noEncrypt(OutputStream stream) throws SocketException{
		try{
			if (packetLength==0)
				calLength();
			ByteArrayOutputStream byteStream=new ByteArrayOutputStream();
